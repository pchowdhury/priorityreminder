    ...
    public Object getDataFromApi() {
        ...
        if (StaticDataProvider.getLiveInstance().isUsingEnableStaticEngine() && !StaticDataProvider.getLiveInstance().shouldGenerateCacheForDebug()) {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
                try {
                    JSONObject json = StaticDataProvider.getLiveInstance().getJSONFor(DataStore.DATA_FILE_NAME);
                    JSONArray jsonArr = null;
                    if (json.has(DataStore.DATA_FILE_NAME)) {
                        jsonArr = json.getJSONArray(DataStore.DATA_FILE_NAME);
                    }
                    results = new ArrayList<>();
                    for (int i = 0; i < jsonArr.length(); i++) {
                        results.add(TaskItem.getTaskFromJSON(jsonArr.getJSONObject(i)));
                    }
            ...
            if (values != null) {
                ...
                if (StaticDataProvider.getLiveInstance().shouldGenerateCacheForDebug()) {
                    ...
                    for (TaskItem project : results) {
                        jsonArr.put(project.toJSON());
                    }
                    JSONObject json = new JSONObject();
                    try {
                        json.put(DataStore.DATA_FILE_NAME, jsonArr);
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                    StaticDataProvider.getLiveInstance().generateMockup(DataStore.DATA_FILE_NAME, json.toString().getBytes());
                    LogUtils.logI("Task Cached", json.toString());
                }
            }
        }
        return results;
